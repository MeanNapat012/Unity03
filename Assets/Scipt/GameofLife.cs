...
public class GameofLife : MonoBehaviour {
    [SerializeField] private TextAsset _stageTextAssert;
    [SerializeField] private GameObject _cellPrefab;
    private CellStatus[,] _cellStatusArray;
    private Cell[,] _cellArray;
    private GamePhaseEnum _gamePhaseEnum = GamePhaseEnum.SETUP;
    public GameObject NextButton;


    void LoadCellStatus() {
        string[] lines = this._stageTextAssert.text.Split("\n");
    ...
    void CreateCell() {
        ...
        for (int i = 0; i < this._cellStatusArray.GetLength(0); i++) {
            for (int j = 0; j < this._cellStatusArray.GetLength(0); j++) {
    ...
    //     this._gamePhaseEnum = GamePhaseEnum.SETUP;
    // }
    ...
    void Update() {
        if (this._gamePhaseEnum == GamePhaseEnum.SETUP) {
    ...
    {
        if(this._gamePhaseEnum == GamePhaseEnum.OVER)
        {
            NextButton.SetActive(false);
        }
        this._gamePhaseEnum = GamePhaseEnum.PLAY;
        ...
        if(this.IsGameOver())
        {
            this._gamePhaseEnum = GamePhaseEnum.OVER;
        }
    }
    bool IsGameOver()
    {
